...
fn redirector(
    psk: String,
    a_stream: mnet::TcpStream,
    listener: std::net::TcpListener,
    tx: oneshot::Sender<PlzDial>,
) -> Result<()> {
...
impl Net {
    ...
    fn remove_client_by_addr(&mut self, addr: &SocketAddr) {
        ...
        let rp_cleanup: Vec<u16> = self
            .tunnel_port_2_client
            .iter()
            .filter(|(_, a)| &addr == a)
            .map(|(port, _)| *port)
            .collect();
    ...
    pub fn cleanup_stale_clients(&mut self) -> Result<()> {
        let mut buf = [0u8; 1];
        let cleanup: HashSet<SocketAddr> = self
            .clients
            .iter_mut()
            .filter(|(_, &mut ref c)| {
    ...
    pub fn accept(&mut self, psk: &String, registry: &Registry) -> stnet::Result<()> {
        ...
        self.insert_client(addr, transport);
        ...
        for tunnel in cc.tunnels {
            ...
            let l: &mut mnet::TcpListener = self.get_tunnel_listener(tunnel.remote_port).unwrap();
...
impl Remote {
    pub fn new(listener: mnet::TcpListener) -> Result<Remote> {
        ...
        c.mio
            .poll
            .registry()
            .register(&mut c.net.listener, Token(0), Interest::READABLE)?;
        Ok(c)
    }
    ...
    pub fn run(&mut self, psk: &String) -> Result<()> {
        ...
        'outer: loop {
            self.mio
                .poll
                .poll(&mut self.mio.events, Some(Duration::new(1, 0)))?;
            if last_check.elapsed() >= check_interval {
                let _ = self.net.cleanup_stale_clients();
                last_check = Instant::now();

                let stale_handlers: Vec<usize> = handlers
                    .iter()
                    .enumerate()
            ...
            for ev in &self.mio.events {
                if ev.token() == Token(0) && ev.is_readable() {
                    ...
                    loop {
                        let token = ev.token();
                        if ev.is_readable() {
                            let addr = *self.net.get_addr(token.0).unwrap();
