    ...
    T: stnet::Stream,
{
    ...
    async fn new_conn(&mut self, id: SocketAddr, port: u16) -> stnet::Result<()> {
        let tunnel_cfg = match self.config.tunnels.get(&port) {
            None => {
                error!(for_ = ?id, "Client is requesting tunnel it never created. Bad client.");
                return Err(stnet::OtherSnafu {
                    message: "misbehaving client",
                }
                .build());
            }
            Some(p) => p,
        ...
        let internal_stream = match tnet::TcpStream::connect(internal_addr).await.with_context(
            |_| stnet::IoSnafu {
                message: format!("failed to connect to {internal_addr:?}"),
            },
        ) {
            Ok(s) => s,
            Err(e) => {
